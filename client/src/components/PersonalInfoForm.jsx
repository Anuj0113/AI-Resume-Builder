...
const PersonalInfoForm = ({data, onChange, removeBackground, setRemoveBackground}) => {
  ...
  const handleChnage=(field,value)=>{
        ...
        <div className='flex items-center gap-2'>
          <label>
            {data.image ? (
              ...
              className='w-16 h-16 rounded-full object-cover mt-5 ring-slate-300 hover:opacity-80'/>
            ) : (
            ...
            <input type="file" accept="image/jpeg, image/png" hidden onChange={(e)=>handleChnage("image", e.target.files[0])} />
          </label>
          {typeof data.image === 'object' && (
            <div className='flex flex-col gap-1 pl-4 text-sm'>
              <p>RemoveBackground</p>
          ...
          fields.map((field)=>{
            ...
            return (
              <div key={field.key} className='space-y-1 mt-5'>
                ...
                <input type={field.type} value={data[field.key] || ""} onChange={(e)=>handleChnage(field.key, e.target.value)}
